      ...
      <div v-for="item in tableData" style="padding: 20px; border-radius: 10px; background-color: #fff; margin: 10px 0" :key="item.id">
        <div style="display: flex">
          ...
          <div style="flex: 1">
            ...
            <el-button type="text" style="float: right; font-size: 16px; color: red; margin-left: 10px" @click="del(item.id)"> delete</el-button>
  ...
  methods: {
    ...
    save() {
      this.request.post("/articleKp", this.form).then(res => {
        if (res.code === '200') {
          this.$message.success("saved successful")
    ...
    delBatch() {
      if (!this.multipleSelection.length) {
        this.$message.error("please select data needed to be deleted")
        return
      }
      let ids = this.multipleSelection.map(v => v.id)  // [{}, {}, {}] => [1,2,3]
      this.request.post("/articleKp/del/batch", ids).then(res => {
        if (res.code === '200') {
          this.$message.success("batch deletion succeeful")
          this.load()
        } else {
