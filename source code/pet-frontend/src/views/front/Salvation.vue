<template>
  <div style="min-height: calc(100vh - 70px)">
    ...
    <div style="margin: 10px 0">
      <el-button type="primary" @click="handleAdd" >new rescue info</el-button>
    </div>

    <el-table :data="tableData" stripe :show-header="false"  style="border-radius: 10px">
      <el-table-column label="pic" width="150"><template slot-scope="scope"><el-image style="width: 100px; height: 100px"
    ...
    <div style="margin: 10px 0">
      <div class="feed"> Pet Feeding Point</div>
      <feed></feed>
    </div>
  ...
  methods: {
    ...
    delBatch() {
      ...
      this.request.post("/salvation/del/batch", ids).then(res => {
        if (res.code === '200') {
          this.$message.success("Batch deletion successfull")
          this.load()
        } else {
